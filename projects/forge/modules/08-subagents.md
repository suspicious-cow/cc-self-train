...
> **STOP** -- Dispatch a small background task (e.g., `claude --bg "list every TODO comment in the toolkit"`). Switch to your `claude agents` terminal, watch it appear and finish, then `claude attach <id>` to read the result.
...
> **STOP** -- Decide which isolation fits your workflow. For a solo dev toolkit, the default worktree isolation is fine -- confirm you understand why `"none"` would be risky with two background sessions running at once.
