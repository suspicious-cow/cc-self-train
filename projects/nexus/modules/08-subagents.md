...
> **STOP** -- Dispatch a small background task (e.g., `claude --bg "list every route handler missing a logging call"`). Switch to your `claude agents` terminal, watch it appear and finish, then `claude attach <id>` to read the result.
...
> **STOP** -- Decide which isolation fits your workflow. For a single-author gateway project, the default worktree isolation is fine -- confirm you understand why `"none"` would be risky with two background sessions running at once.
